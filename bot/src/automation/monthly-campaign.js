...
async function expandWithAi({ missing, report, occupiedTitles, ai }) {
  ...
  const response = await ai.generate(
    ...
    `Crie exatamente ${missing} pautas editoriais novas e não sobrepostas. Use os rankings globais, as pautas derivadas e os sinais abaixo apenas como inteligência, nunca como prova factual. Priorize recorrência entre mercados, intenção de busca e lacunas do acervo; não repita palavras-chave artificialmente. Evite estes títulos já usados: ${JSON.stringify(occupiedTitles)}. Inteligência: ${JSON.stringify({ globalRankings: report.globalRankings || {}, briefs: report.briefs, marketSignals: report.marketSignals?.slice(0, 20) || [] })}. Formato: {"topics":[{"id":"slug","title":"20 a 140 caracteres","summary":"40 a 260 caracteres","category":"manutencao-ajustes|engenharia|review|comparativo|componentes|lancamentos|competicoes","freshness":"evergreen|revalidate-24h|event-driven"}]}`,
    {
      jsonMode: true,
      temperature: 0.2,
      maxTokens: 7000,
