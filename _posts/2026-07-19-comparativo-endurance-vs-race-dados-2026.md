---
layout: post
title: "Endurance vs Race: comparativo completo com dados reais do mercado brasileiro em 2026"
description: "Qual perfil de bike é ideal para você? Comparamos dados reais de geometria, peso, preço e componentes da base Pedal Data."
date: 2026-07-19
last_modified_at: 2026-07-19
author: "Equipe Pedal Data"
content_type: "comparativo"
review_method: "desk-research"
tested_by_pedaldata: false
ai_assisted: true
editorial_status: "published"
thumbnail: "/assets/img/posts/endurance-vs-race-dados/thumb-480.webp"
image: "/assets/img/posts/endurance-vs-race-dados/hero.jpg"
image_credit: "Pedal Data"
image_license: "Uso editorial do Pedal Data"
tags: [comparativo, dados]
reading_time: 8
---

A escolha entre uma bike endurance e uma race é uma das decisões mais importantes para quem compra uma bike de estrada. Com base na base de dados do Pedal Data, comparamos as opções disponíveis no mercado brasileiro em 2026.

## O que dizem os dados

### Scott Addict 20 vs Specialized Tarmac SL7

| Critério | Scott Addict 20 (Endurance) | Specialized Tarmac SL7 (Race) |
|---|---|---|
| Preço | R$ 18.990 | R$ 24.990 |
| Peso | 8,4 kg | 8,8 kg |
| Quadro | Carbono HMF | Carbono FACT 10r |
| Grupo | 105 Di2 (12v) | 105 Di2 (12v) |
| Transmissão | Eletrônico | Eletrônico |
| Pneus | 30 mm | 26 mm |
| Stack (M) | 566 mm | 544 mm |
| Reach (M) | 384 mm | 385 mm |
| Stack/Reach | 1,47 | 1,41 |
| Clearance | 35 mm | 32 mm |

### Trek Domane SL5 vs Cannondale SuperSix Evo 105

| Critério | Trek Domane SL5 (Endurance) | Cannondale SuperSix Evo 105 (Race) |
|---|---|---|
| Preço | R$ 21.990 | R$ 14.850 |
| Peso | 9,2 kg | 8,6 kg |
| Quadro | Carbono OCLV 500 | Carbono BallisTec |
| Grupo | 105 Di2 (12v) | 105 Di2 (12v) |
| Pneus | 32 mm | 28 mm |
| Clearance | 38 mm | 30 mm |

## Para quem cada perfil é ideal

### Escolha Endurance se:
- Você pedala mais de 2h seguidas
- Prioriza conforto em longas distâncias
- Quer pneus mais largos (30-32mm) para absorção
- Tem dores nas costas ou pescoço ao pedalar
- Faz cicloturismo e pedalos de final de semana

### Escolha Race se:
- Seu foco é performance e velocidade
- Participa de provas e treinos intensos
- Prefere resposta imediata nas acelerações
- Já tem boa flexibilidade e experiência
- Não se incomoda com posição mais agressiva

## Conclusão da base de dados

Analisando as **30 bikes do catálogo**, observamos que:

- **Endurances são 22 mm mais altas** (stack) que as race — diferença significativa para conforto
- O **peso médio** é praticamente igual entre os perfis (8,7 kg endurance vs 8,6 kg race)
- **Endurances aceitam pneus até 38mm**, enquanto a maioria das race limita em 30-32mm
- O **preço médio** das endurance (R$ 21.000) é similar ao das race (R$ 22.000)

> *Dados da base Pedal Data — 30 bicicletas catalogadas em julho/2026. Preços podem variar conforme loja e região.*
